function finite_cdf2(beta;initial_time = 13.0, final_time = -10.0, delta_x = -0.01, delta_theta = 0.001*pi)
    time=initial_time:delta_x:final_time;
    final_theta=5*pi-delta_theta; domain=0:delta_theta:final_theta; domain=domain[2:end-1];
    ...
    for k=1:length(time)-1
        ...
        final=lhs\rhs;
        final_interest[k+1]=final[round(Int,2*pi/delta_theta)];
    end
    ϕ = x -> (erf.(x) .+ 1.0)/2;
